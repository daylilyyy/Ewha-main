...
const mainInit = () => {
  const mainVisual = () => {
    // .main #visual
    const $visualBannerBox = get(`#visual .bg-box`),
    ...
    // .main #visual
    let slideArr = ["-100%", "-200%", "-300%"];
    let visualCurrentCnt = 0,
      visualPrevCnt = 0,
      visualLen = $visualPagingLi.length,
  ...
  const mainStory = () => {
    // .main .story
    const $storyBannerBox = get(".main .story .rolling-box"),
      $storyPagingLi = getAll(".main .story .paging-box .paging li"),
      $storyActiveBtn = get(".main .story .paging-box .active-btn");
    // .main .story
    let storySlideArr = ["0", "-50%", "-100%", "-150%"];
    let storyCurrentCnt = 0,
      storyPrevCnt = 0,
      storyIntervalId = null,
      storyIsRolling = true;
    // .main .story
    const storyRollingBanner = () => {
      // 배너
      if (storyCurrentCnt === 0 && storyPrevCnt === 3) {
        $storyBannerBox.style.left = "-200%";
        setTimeout(() => {
          $storyBannerBox.style.transition = "0s";
          $storyBannerBox.style.left = storySlideArr[storyCurrentCnt];
        }, 400);
      } else {
        $storyBannerBox.style.transition = "0.4s";
        $storyBannerBox.style.left = storySlideArr[storyCurrentCnt];
      }
      // 페이징
    ...
    const storyRolling = () => {
      storyCurrentCnt = storyCurrentCnt >= $storyPagingLi.length - 1 ? 0 : storyCurrentCnt + 1;
      storyRollingBanner();
    };
    // .main .story
  ...
  const mainBucheonNow = () => {
    // .main .bucheon-now
    const $bucheonNowContent = get(".main .con-box.bucheon-now"),
      $bucheonNowRollingBox = get(".main .bucheon-now .rolling-container .rolling-box"),
  ...
  const common = () => {
    ...
    const quickMenuTextAnimate = () => {
      $quickMenuTextBgText.forEach((item, idx) => {
        if (idx % 2 === 0) {
          item.animate(
            [{ transform: "translateX(100%)" }, { transform: `translateX(-300%)` }, { transform: "translateX(100%)" }],
            100000
          );
        } else {
          item.animate(
            [{ transform: `translateX(-300%)` }, { transform: "translateX(100%)" }, { transform: `translateX(-300%)` }],
            100000
          );
        }
      });
...
const facilityInformationPageInit = () => {
  const $facilityInformationBgText = get(
    ...
    $facilityInformationBgTextList = get(
      ".facility-information-page .facility-information .facility-information-Bgtext-box .facility-information-BgText li"
    );
  let facilityInformationStep = 4,
    facilityInformationSize = 0,
    facilityInformationTextX = 0,
    facilityInformationLeftX;
